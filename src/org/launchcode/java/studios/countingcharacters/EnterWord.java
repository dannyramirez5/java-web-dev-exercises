package org.launchcode.java.studios.countingcharacters;

import java.util.HashMap;

public class EnterWord {
    public static void main(String[] args) {
        String hiddenFigures = "If the product of two terms is zero " +
                "then common sense says at least one of the two terms has to be " +
                "zero to start with. So if you move all the terms over to one side, " +
                "you can put the quadratics into a form that can be factored allowing " +
                "that side of the equation to equal zero. Once you’ve done that, " +
                "it’s pretty straightforward from there.";

        //ArrayList<Character> hf = new ArrayList<>();

        HashMap<Character, Integer> characterCounts = new HashMap<>();

        //char count = CountString.characterCount(hiddenFigures);
        for (char c : hiddenFigures.toCharArray()) {
            if (characterCounts.containsKey(c)) {
                characterCounts.put(c, characterCounts.get(c) + 1);
            }
            else {
                characterCounts.put(c, 1);
            }
        }
        System.out.println(characterCounts);
    }
}
